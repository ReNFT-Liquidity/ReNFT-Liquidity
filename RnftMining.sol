...
contract RnftRewardBabyPools is Ownable {
    using SafeMath for uint256;
    ...
    constructor(IERC20 _baby) public {
        rewardToken = _baby;
        precision = 1e18;
    ...
    function getpools() view public returns(PoolInfo[] memory){
        return poolinfo;
    }

    function getBlockNumber() view public returns(uint256){
        return block.number.sub(startBlockNumber);
    }

    function getUnproducedBlocks(uint256 _pid) public validatePool(_pid) view returns(uint){
        PoolInfo storage pool = poolinfo[_pid];
        uint256 totalTime = pool.endtime.sub(pool.starttime);
        uint256 totalBlocks = totalTime.div(3);
        if(block.timestamp < pool.starttime){
            return 0;
        }else{
            uint256 passTime = block.timestamp.sub(pool.starttime);
            uint256 passBlock = passTime.div(3);
            uint256 result;
            if(passBlock > totalBlocks){
                result = 0;
            }else{
                result = totalBlocks.sub(passBlock);
            }
            return result;
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePool(_pid){
        ...
        reclaimAppleStakingReward(_pid);
    }
    ...
    function safeAppleTransfer(address _to, uint256 _amount) internal {
        uint256 AppleBalance = rewardToken.balanceOf(address(this));
